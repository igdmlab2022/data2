...
def GA(loc, matrix, demands, capacity):
    ...
    for g in range(NUM_generation):
        ...
        for _ in range(int(POPULATION_size/(2 * cpu_count) )):
...
if __name__ == "__main__":
    ...
    parser.add_argument('-o', '--offset', metavar='OF', type=int, default=0,
                        help='offset of the dataset')
    ...
    run_time = 0
    iteration = 0
    out_file = 'log_GA_VRP' + str(size) + '-off' + str(offset) + '-' + str(time()) + '.txt'
    if size == 10:
        POPULATION_size = 192
        NUM_generation = 350
        NUM_vehicles = 5
